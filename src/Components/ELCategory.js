import React, { Component, Fragment } from 'react';
import SpanElement from './SpanElement';
import BlockLabel from './BlockLabel';
import {withTheme} from './theme-context';

const ThemedLabel = withTheme(BlockLabel);

/*generally appears at the top/bottom of each card to make it clear which deck the learner is currently using.
this may be substituted with a country flag or other visual indicator such as a logo or Unicode symbol */
//todo: implement expert mode switch which doesn't even render any components, just text
class ELCategory extends Component {
	render() {
		const { theme, cardLang, cardType, labelOn, isForExport, size, importance } = this.props;
		const themeStyle = (theme === "black-board") ? "tertiary-color pastel-chalk-text" : "";

		return (
			<div className="card-element card-category">
				{
					!!labelOn &&
					<ThemedLabel
						text="Language & Card Type"
						theme={theme}
						additionalClass={themeStyle}
					/>}
				<div className={`${themeStyle} card-element-line ${importance} ${size} special-field-category`} >
					{
						!isForExport &&
						<Fragment>
							<SpanElement
								field={`${cardLang}::${cardType}`}
							/>
						</Fragment>
					}
					{
						!!isForExport &&
						<span>{`{{Subdeck}}::{{Card}}`}</span>
					}
				</div>
			</div>
		);
	}
};

export default ELCategory;
